...
@Component({
  selector: 'novelties-upload-image-common',
...
export class UploadImageComponentCommon implements OnInit {

  public uploadForm!: FormGroup;
  @Output() siguientePaso = new EventEmitter<void>();
  @Output() eventUpdateFiles =  new EventEmitter<void>();
  @Input()
  events!: Observable<void>;
  disabledCarga: boolean = false;

  @Input() tipoFormato: any;
  @Input() tipoCantidadImagen: any;
  @Input() id_asesor: any;
  sizeMax: any;
  ...
  constructor(public dialog: MatDialog, private parameterRepository: ParameterRepository, private noveltiesRepositry: NoveltiesRepository,
    public store: StoreService, private _snackBar: MatSnackBar) {
  }
  ...
  formatImages() {
    ...
    this.sizeMax = Utils.convertBytestoMegaBytes(this.maxFileSize);
  ...
  openDialog() {
    ...
    const dialogRef = this.dialog.open(DialogContentCameraComponentCommon, dialogConfig);
  ...
  messageErrorSize(size:number) {
    ...
    if(size==1) {
      textoMessage =  'El máximo de imágenes permitido es '+this.cantImage;
    } else {
  ...
  changeValueFiles() {
    this.store.setFiles(this.files);
    this.eventUpdateFiles.emit();
  }
...
export class DialogContentCameraComponentCommon {
  [x: string]: any;
  ...
  maxFileSize: number = 0;
  constructor(
    public dialogRef: MatDialogRef<DialogContentCameraComponentCommon>,
